...
void draw_bbox_with_label(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch, 
    ...
    for (size_t batch_i = 0; batch_i < img_batch.size(); batch_i++) {
        auto& res = res_batch[batch_i];
        cv::Mat img = img_batch[batch_i];
        
        for (size_t det_j = 0; det_j < res.size(); det_j++) {
            auto& detection = res[det_j];
            
            // 修复：将 class_id 转换为整数再进行取模运算
            int class_id_int = static_cast<int>(detection.class_id);
            auto color = colors[class_id_int % colors.size()];
            auto bgr = cv::Scalar(color & 0xFF, color >> 8 & 0xFF, color >> 16 & 0xFF);
            
            // 绘制边界框
            cv::Rect r = get_rect(img, detection.bbox);
            cv::rectangle(img, r, bgr, 2);
            
            // 准备文本
            std::string label_text = labels_map[class_id_int];
            
            // 计算文本大小
            int baseline = 0;
            cv::Size text_size = cv::getTextSize(label_text, cv::FONT_HERSHEY_PLAIN, 1.2, 2, &baseline);
            
            // 绘制文本背景
            cv::Point text_bg_tl(r.x, r.y - text_size.height - 5);
            cv::Point text_bg_br(r.x + text_size.width, r.y);
            cv::rectangle(img, text_bg_tl, text_bg_br, bgr, -1);  // -1 表示填充
            
            // 绘制文本
            cv::Point text_pos(r.x, r.y - 5);
            cv::putText(img, label_text, text_pos, cv::FONT_HERSHEY_PLAIN, 1.2, 
                       cv::Scalar(255, 255, 255), 2);  // 白色文本
        }
    }
}
