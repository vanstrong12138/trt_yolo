...
void prepare_buffer(ICudaEngine* engine, float** input_buffer_device, float** output_buffer_device,
    ...
    CUDA_CHECK(cudaMalloc((void**)input_buffer_device, input_size));
    CUDA_CHECK(cudaMalloc((void**)output_buffer_device, output_size));
...
void infer(IExecutionContext& context, cudaStream_t& stream, float* input_buffer_device, float* output_buffer_device, 
    ...
    if (cuda_post_process == "c") {
        ...
        cuda_decode(output_buffer_device, model_bboxes, kConfThresh, decode_ptr_device, kMaxNumOutputBbox, stream);
...
class YOLOv11ROSNode : public rclcpp::Node {
public:
    YOLOv11ROSNode() : Node("yolov11_node") {
        ...
        this->declare_parameter<std::string>("output_topic", "/yolov11/detections");
...
private:
    void initialize() {
        RCLCPP_INFO(this->get_logger(), "Initializing YOLOv11 ROS2 Node...");
    ...
    void initializeInference(std::string& engine_path) {
        ...
        image_sub_ = image_transport::create_subscription(
            this,
            image_topic,
            std::bind(&YOLOv11ROSNode::imageCallback, this, std::placeholders::_1),
            "raw",
            rmw_qos_profile_sensor_data
        );
        
        detection_pub_ = this->create_publisher<vision_msgs::msg::Detection2DArray>("/yolov11/detections", 10);
        
        if (publish_image) {
            image_pub_ = image_transport::create_publisher(this, "/yolov11/detection_image");
        }
        
        RCLCPP_INFO(this->get_logger(), "YOLOv11 ROS2 Node inference mode initialized successfully");
    }
    
    void cleanup() {
        RCLCPP_INFO(this->get_logger(), "Cleaning up YOLOv11 ROS2 Node...");
        ...
        RCLCPP_INFO(this->get_logger(), "YOLOv11 ROS2 Node cleanup completed");
    }
    ...
    void imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
        try {
            ...
            cuda_batch_preprocess(img_batch, input_buffer_device_, kInputW, kInputH, stream_);
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<YOLOv11ROSNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
