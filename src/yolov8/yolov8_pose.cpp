...
// ROS2 YOLOv8姿态估计节点类
class YOLOv8PoseROSNode : public rclcpp::Node {
public:
    YOLOv8PoseROSNode() : Node("yolov8_pose_node") {
        // 从参数服务器获取参数
        ...
        this->declare_parameter<std::string>("model_type", "n");  // n/s/m/l/x
        this->declare_parameter<std::string>("cuda_post_process", "c");
        this->declare_parameter<std::string>("image_topic", "/camera/image_raw");
        this->declare_parameter<std::string>("output_topic", "/yolov8/pose_detections");
...
private:
    void initialize() {
        RCLCPP_INFO(this->get_logger(), "Initializing YOLOv8 Pose ROS2 Node...");
    ...
    void serializeModel(const std::string& wts_path, const std::string& engine_path, const std::string& model_type) {
        RCLCPP_INFO(this->get_logger(), "Starting pose model serialization...");
        ...
        if (model_type.size() == 2) {
            if (model_type[1] == '6') {
                ...
                is_p = 2;
                RCLCPP_WARN(this->get_logger(), "p2 is not supported right now");
            }
        }
        ...
        RCLCPP_INFO(this->get_logger(), "Pose engine serialized successfully to: %s", engine_path.c_str());
    ...
    void initializeInference(std::string& engine_path) {
        RCLCPP_INFO(this->get_logger(), "Starting pose inference mode initialization...");
        ...
        image_sub_ = image_transport::create_subscription(
            this,
            image_topic,
            std::bind(&YOLOv8PoseROSNode::imageCallback, this, std::placeholders::_1),
            "raw",
            rmw_qos_profile_sensor_data
        );
        
        // 创建检测结果发布者
        detection_pub_ = this->create_publisher<vision_msgs::msg::Detection2DArray>("/yolov8/pose_detections", 10);
        
        if (publish_image) {
            image_pub_ = image_transport::create_publisher(this, "/yolov8/pose_detection_image");
        }
        
        RCLCPP_INFO(this->get_logger(), "YOLOv8 Pose ROS2 Node inference mode initialized successfully");
    }
    
    void cleanup() {
        RCLCPP_INFO(this->get_logger(), "Cleaning up YOLOv8 Pose ROS2 Node...");
        ...
        RCLCPP_INFO(this->get_logger(), "YOLOv8 Pose ROS2 Node cleanup completed");
    }
    ...
    void imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
        try {
            ...
            if (cuda_post_process_ == "c") {
                ...
                RCLCPP_WARN(this->get_logger(), "pose_postprocess is not support in gpu right now");
                return;
            }
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<YOLOv8PoseROSNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
